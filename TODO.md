...
```
src/
├── types/index.ts           # 완전한 타입 시스템
├── services/               
│   ├── index.ts            # ServiceContainer (싱글톤)
│   ├── firestore.ts        # Firestore 연동 (대화 세션/메시지 관리)
│   ├── qdrant.ts           # Qdrant 연동 (벡터 검색/임베딩)
│   ├── openai.ts           # OpenAI 연동 (임베딩/채팅/요약)
│   ├── telegram.ts         # Telegram 연동 (봇/웹훅)
│   └── github.ts           # GitHub 연동 (파일/웹훅)
├── utils/                  # 유틸리티 함수들
├── controllers/health.ts   # Health check 컨트롤러
